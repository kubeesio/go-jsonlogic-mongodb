...
func TestConvert(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "convert 3 comparisons joined by and example", args: args{rules: strings.NewReader(`{"and": [{"!=": [1, 2]}, {"==": [1, 1]}, {"==": [3, 3]}]}`)}, want: bson.D{{Key: "$and", Value: bson.A{bson.D{{Key: "$ne", Value: bson.A{1.0, 2.0}}}, bson.D{{Key: "$eq", Value: bson.A{1.0, 1.0}}}, bson.D{{Key: "$eq", Value: bson.A{3.0, 3.0}}}}}}, wantErr: false},
